...
def get_current_month_and_year():
    now = datetime.now()
    month_num = now.month
    months_ui = {1: "январь", 2: "февраль", 3: "март", 4: "апрель", 5: "май", 6: "июнь",
...
async def js_click_by_text(page, text):
    try:
        ...
        """, text)
        if result: print(f"✅ Клик: {text}")
        else: print(f"⚠️ Не найден: {text}")
        return result
    except Exception as e:
...
def format_telegram_message(added, removed, changed, month_name):
    msg = f"🚨 <b>Изменения в расписании ({month_name.capitalize()})</b>\n\n"
    ...
    if changed:
        ...
        for new_e, diffs in changed:
            msg += f"• {new_e['date']} в {new_e['time_start']}:\n"
            for d in diffs:
                msg += f"  {d}\n"
        msg += "\n"
...
async def main():
    ...
    for cell in schedule_cells:
        try:
            ...
            events_by_date[date_fmt].append({
                "time_start": time_parts[0], "time_end": time_parts[1],
    ...
    for date_fmt, day_events in events_by_date.items():
        ...
        for slot_number, event in enumerate(day_events, start=1):
            ...
            final_events.append({
                "date": date_fmt, "time_start": event['time_start'], "time_end": event['time_end'],
                "title": title, "subject_name": event['subject_name'], 
                "room": event['room'], "teacher": event['teacher']
            })
    ...
    added, removed, changed = get_diff(old_events, final_events)
    ...
    if not added and not removed and not changed:
        ...
        with open('schedule.ics', 'wb') as f: f.write(new_ics_data)
        with open('schedule.json', 'w', encoding='utf-8') as f: json.dump(final_events, f, ensure_ascii=False, indent=2)
        
        msg = format_telegram_message(added, removed, changed, current_month_ui)
        send_telegram(msg)
